/**
 * Dynarch Horizontal Menu, hmenu-2.9 Copyright Dynarch.com, 2003-2006. All
 * Rights Reserved. http://www.dynarch.com/products/dhtml-menu/
 * 
 * THIS NOTICE MUST REMAIN INTACT!
 * 
 * LICENSEE: mvdm.org License key: 9259c351fa2b50585f6bd26f13ca8a65 Purchase
 * date: Mon Jul 24 08:33:35 2006 GMT License type: developer
 * 
 * For details on this license please visit the product homepage at the URL
 * above.
 */
( function() {
	...
	if (typeof _dynarch_top._dynarch_menu_url == "undefined")
		_dynarch_top._dynarch_menu_url = "/img/hmenu/";
	else {
		_dynarch_top._dynarch_menu_url = _dynarch_top._dynarch_menu_url
				.replace(/\x2f*$/, '/');
		if (!/^(https?:|\x2f)/.test(_dynarch_top._dynarch_menu_url)
				&& _dynarch_top != window)
			_dynarch_top._dynarch_menu_url = _dynarch_top.document.URL.replace(
					/\x2f?[^\x2f]*$/, "/")
...
function DynarchMenu(el, config) {
	...
	if (config.d_profile)
		alert("DynarchMenu: generated in "
				+ (((new Date()).getTime() - T1) / 1000) + " sec.\n"
				+ "containing " + DynarchMenu.profile.item + " items in "
				+ DynarchMenu.profile.tree + " (sub)menus.");
	if (config.setFocus)
...
DynarchMenu._createHider = function(win) {
	...
	if (is_ie && !is_ie5) {
		...
		win.document.body.insertAdjacentHTML('beforeEnd', '<iframe id="' + id
...
DynarchMenu._forAllMenus = function(callback) {
	for ( var i = _dynarch_top.DynarchMenu._menus.length; --i >= 0
			&& !callback(_dynarch_top.DynarchMenu._menus[i]);)
		;
};
...
DynarchMenu.prototype.addIcon = function(info, item, icon) {
	var doc = info.parent.base ? this.config.frames.main.document
			: this.config.frames.popups.document;
	var CE = DynarchMenu._createElement, t, l = item.firstChild.firstChild, r = CE(
			"tr", CE("tbody", t = CE("table", null, doc))), td1 = CE("td", r), td2 = CE(
			"td", r);
...
DynarchMenu.prototype.createMenuItem = function(li, parent, horiz, arrow) {
	var tmp, ctx = null, cfg = this.config, licl = li.className, icon = null, label, html_popup = true, tooltip, action = null, item, info, self = this, key = null, disabled = /(^|\s+)disabled(\s+|$)/i
			.test(licl), nohover = DynarchMenu.getCA(li, "nohover"), win = parent.base ? cfg.frames.main
			: cfg.frames.popups, doc = win.document, CE = DynarchMenu._createElement;
	if (cfg.d_profile)
	...
	info = new DynarchMenu.MenuItem( {
		...
		disabled :disabled,
		nohover :nohover,
		align :licl ? (DynarchMenu._RE_CTX_AL.test(licl) ? RegExp.$1 : "mouse")
				: "mouse"
	});
	if (li.id)
...
DynarchMenu._documentMouseDown = function(ev) {
	...
	if (!j || j.__msh_info.base)
		_dynarch_top.DynarchMenu._closeOtherMenus(j && j.__msh_info.menu);
};
	...
	if (menu && menu.config.electric) {
		...
		if (!el || el === document.body) {
			...
			if (tmout === true)
				tmout = 1;
			if (!_dynarch_top.DynarchMenu._T)
				_dynarch_top.DynarchMenu._T = _dynarch_top
						.setTimeout(
								'_dynarch_top.DynarchMenu._closeOtherMenus(null); _dynarch_top.DynarchMenu._T = null;',
								tmout);
		} else
	...
	DynarchMenu._forAllMenus( function(menu) {
		...
		function serveKeymap(keymap) {
			var key = String.fromCharCode(
					(is_ie || is_opera) ? ev.keyCode : ev.charCode)
					.toLowerCase();
			item = keymap[key];
...
DynarchMenu.prototype.createMenuTree = function(ul, horiz) {
	var base = !this._baseMenuInfo, a_li, div, table, i, info, li, item, tmp, ret = null, self = this, cfg = this.config, ctx = cfg.context, submenu, CE = DynarchMenu._createElement, win = base ? cfg.frames.main
	...
	div.className = (base && horiz) ? "dynarch-horiz-menu"
			: "dynarch-popup-menu";
	...
	info = new DynarchMenu.MenuTree( {
		...
		config :cfg,
		_T_close :null,
		open_left :DynarchMenu._RE_OPL.test(ul.className)
	});
	if (ul.id)
		info.id = ul.id;
	...
	try {
		...
		if (a.length == 0) {
			...
			for (i = a.length; --i >= 0;) {
				...
				if (el)
					try {
						DynarchMenu._removeEvent(el,
								(is_ie || is_opera) ? "keydown" : "keypress",
								el.DynarchMenu._documentKeyPress);
						DynarchMenu._removeEvent(el, "mousedown",
								el.DynarchMenu._documentMouseDown);
						DynarchMenu._removeEvent(el, "mouseup",
								el.DynarchMenu._documentMouseUp);
						DynarchMenu._removeEvent(el, "mouseover",
								el.DynarchMenu._documentMouseOver);
					} catch (e) {
					}
				;
			}
...
DynarchMenu.addInfo = function(el, name, value) {
	...
	if (is_ie) {
		...
		if (!a) {
			...
			DynarchMenu._addEvent(_dynarch_top, "unload",
					_dynarch_top.DynarchMenu._cleanUp);
		}
		a[a.length] = [ el, name ];
	}
};
...
DynarchMenu.MenuItem.prototype.disable = function(dis) {
	if (typeof dis == "undefined")
		dis = true;
	this.disabled = dis;
	DynarchMenu._class(this.element, DynarchMenu._RE_DS, dis ? "disabled"
			: null);
};
...
DynarchMenu.MenuItem.prototype.exec = function() {
	...
	var timer = setInterval( function() {
		DynarchMenu._class(self.element, DynarchMenu._RE_AH,
				--step & 1 ? 'active' : null);
		if (!step) {
			clearInterval(timer);
			self._exec();
...
DynarchMenu.MenuItem.prototype.hover = function(activate, instant) {
	...
	if (typeof activate == "undefined")
		activate = this.submenu
				&& (menu.config.electric || !menu.base || menu.active_submenu);
	menu.clearPopups(this);
...
DynarchMenu.MenuItem.prototype.activate = function(noclose, instant) {
	if (!this.disabled) {
		...
		if (submenu) {
			...
			if (!noclose && !menu.config.electric && menu.base
					&& submenu == menu.active_submenu) {
...
DynarchMenu.MenuTree.prototype.resetActive = function(item, cls) {
	...
	DynarchMenu._class(item, DynarchMenu._RE_AH,
			_dynarch_top.DynarchMenu._activeItem == item ? "active" : cls);
...
DynarchMenu.MenuTree.prototype.getFirstItem = function() {
	return this.horiz ? this.element.firstChild.__msh_info
			: this.table.firstChild.firstChild.__msh_info;
};
DynarchMenu.MenuTree.prototype.getLastItem = function() {
	return this.horiz ? this.element.lastChild.__msh_info
...
DynarchMenu.MenuTree.prototype.openContext = function(ev, trigger) {
	...
	default:
		p = {
			x :ev.clientX + DynarchMenu.psLeft(),
			y :ev.clientY + DynarchMenu.psTop()
		};
		break;
	}
...
DynarchMenu.MenuTree.prototype._scrollIntoView = function(item) {
	...
	if (div.scrollTop > el.offsetTop) {
		...
		diff = el.offsetTop + el.offsetHeight
...
DynarchMenu.MenuTree.prototype._scrollStartHandler = function(dir, div) {
	...
	this._scrollTimer = setInterval(DynarchMenu.$(this._scrollHandler, this,
			dir), this.menu.config.scrolling.speed);
	div.className += " dynarch-menu-scroll-hover "
			+ (dir > 0 ? "dynarch-menu-scroll-down-hover"
...
DynarchMenu.MenuTree.prototype._setupScroll = function() {
	...
	this._scrollDiv2.className = this._scrollDiv2.className.replace(
			DynarchMenu._RE_SCROLL_D, " ");
};
...
DynarchMenu.MenuTree.prototype._open = function(el, item, pos) {
	...
	var m = this.element, self = this.menu, cfg = self.config, win = cfg.frames.popups, p = el ? win.DynarchMenu
			._getPos(el)
			: pos, pe, base = this.parent ? this.parent.base : false, dx = base ? cfg.basedx
			: cfg.dx, dy = base ? cfg.basedy : cfg.dy, horiz = this.parent ? this.parent.horiz
			: false, tmp, s, vw, sw;
	if (!el)
		el = {
			offsetHeight :0,
	...
	if (p.y + m.offsetHeight > vw.y && pe.y > m.offsetHeight) {
		p.y = pe.y
				- m.offsetHeight
				+ (horiz ? 0
						: (win.DynarchMenu._getPos(m).y + m.offsetHeight - win.DynarchMenu
								._getPos(this.getLastItem().element).y));
		dy = -dy;
	} else if (!horiz)
		p.y -= win.DynarchMenu._getPos(this.getFirstItem().element).y
				- win.DynarchMenu._getPos(m).y;
	...
	s.left = p.x + "px";
	s.top = p.y + "px";
	DynarchMenu._setupHider(this.hider, p.x, p.y, m.offsetWidth + sw[0],
			m.offsetHeight + sw[1]);
	if (this.parent) {
	...
	if (cfg.shadows) {
		if (!tmp) {
			...
			this._shadow = tmp = DynarchMenu._createElement(
					(SS && !is_ie) ? "img" : "div", null,
					cfg.container.ownerDocument);
			if (SS)
			...
			if (SS && is_ie && !is_ie5) {
				...
				tmp.runtimeStyle.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
						+ _dynarch_menu_shadow.src + "',sizingMethod='scale')";
			}
			tmp.style.width = "2px";
...
DynarchMenu._nfo = {
	product :"hmenu-2.9",
	licensee :"mvdm.org",
	license_key :"9259c351fa2b50585f6bd26f13ca8a65",
	purchase_date :"Mon Jul 24 08:33:35 2006 GMT",
